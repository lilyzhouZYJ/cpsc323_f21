...
int generate_memory_map(const source_info* info, memory_map* mm)
{
  ...
  // virtual addresses:
  Elf_Xword addr = 0x400000;
  mm->segments[SEG_TEXT].vaddr = addr;

  for(int i = SEG_RODATA; i <= SEG_BSS; i++) {
    addr += mm->segments[i-1].size;
    if((addr & 0xFFF) != 0x0)
      addr = (addr & ~0xFFF) + 0x1000;
    mm->segments[i].vaddr = addr;
  }

  // alignment:
  for(int i = 0; i < SEG_COUNT; i++) {
    mm->segments[i].align = 0x1000;
    // printf("alignment is :%x\n", info->main_sections[i].align);
  }

  // flags:
  mm->segments[SEG_TEXT].flags = PF_X;
  mm->segments[SEG_DATA].flags = PF_R | PF_W;
  mm->segments[SEG_BSS].flags = PF_R | PF_W;
...
int allocate_memory_map(const source_info* src_info, memory_map* mm)
{
  ...
  addr = (uintptr_t) mmap((void*) addr, mm->segments[SEG_TEXT].size, PROT_WRITE, MAP_PRIVATE | MAP_ANON | MAP_32BIT, -1, 0);
  mm->segments[SEG_TEXT].vaddr = addr;

  // RODATA
  addr += mm->segments[SEG_TEXT].size;
  if((addr % 0x1000) != 0)
    addr = (addr & ~0xFFF) + 0x1000;
  addr = mm->segments[SEG_RODATA].size == 0 ? addr : (uintptr_t) mmap((void*) addr, mm->segments[SEG_RODATA].size, PROT_WRITE, MAP_PRIVATE | MAP_ANON | MAP_32BIT, -1, 0);
  mm->segments[SEG_RODATA].vaddr = addr;

  // DATA + padding + BSS

  // start of DATA segment:
  addr += mm->segments[SEG_RODATA].size;
  if((addr % 0x1000) != 0) 
    addr = (addr & ~0xFFF) + 0x1000;
  // end of DATA segment:
  uintptr_t endAddr = addr + mm->segments[SEG_DATA].size;

  // start of BSS address needs to be properly aligned
  uintptr_t startBSS = endAddr;
  int align = src_info->main_sections[SEG_BSS].align;
  int rem = startBSS % align;
  if(rem != 0)
    startBSS = startBSS + (align - rem);
  // size of the padding added
  int padding = startBSS - endAddr;

  // total size for Data + padding + BSS
  int totalSize = mm->segments[SEG_DATA].size + padding + mm->segments[SEG_BSS].size;

  // allocate
  addr = totalSize == 0 ? addr : (uintptr_t) mmap((void*) addr, totalSize, PROT_WRITE, MAP_PRIVATE | MAP_ANON | MAP_32BIT, -1, 0);  

  // set mm structure
  mm->segments[SEG_DATA].vaddr = addr;
  mm->segments[SEG_BSS].vaddr = addr + mm->segments[SEG_DATA].size + padding;
...
int load(ELFIO::elfio* writer, dest_info* dst_info, memory_map* mm)
{
  ...
  // add protection:
  mprotect((void*)mm->segments[SEG_TEXT].vaddr, mm->segments[SEG_TEXT].size, PROT_EXEC | PROT_READ);
  mprotect((void*)mm->segments[SEG_RODATA].vaddr, mm->segments[SEG_RODATA].size, PROT_READ);
  mprotect((void*)mm->segments[SEG_DATA].vaddr, mm->segments[SEG_DATA].size, PROT_READ | PROT_WRITE);
  mprotect((void*)mm->segments[SEG_BSS].vaddr, mm->segments[SEG_BSS].size, PROT_READ | PROT_WRITE);
